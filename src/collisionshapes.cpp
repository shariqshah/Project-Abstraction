...
CollisionMesh::CollisionMesh(CModel* model, bool isTriMesh)
{
	mModel   = model;
	mTriMesh = isTriMesh;
	initialize();
...
{
	if(mModel && mModel->vertices.size() > 0)
	{
		std::vector<Vec3>* vertices = &mModel->vertices;		
		...
		{
			std::vector<unsigned int>* indices = &mModel->indices;
